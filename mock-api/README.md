# mock-api — заглушка API остатков и заказов 1С
...
## Эндпоинты
- `GET /health` → `{"ok": true, "items": <N>, "orders": <M>}`.
- `GET /api/stock?item=<товар>` — остаток по названию.
- `POST /api/stock` body `{"item": "<товар>"}` — то же через POST.
- `POST /api/orders` body `{"item": "<товар>", "quantity": <целое>, "warehouse": null}` —
  оформить заказ (кейс «заказ частей»): назначает номер `ЗР-NNNNNNN`,
  статус «Потребность зарегистрирована»; нулевой остаток → «заказ поставщику».
...
```
Реальный заказ в 1С создаётся через `/api/execute_code` тулкита
(см. docs/1C_INTEGRATION.md) и возвращает тот же набор полей.

## Товары (ключи lowercase)
молоко, хлеб, сахар, соль, кофе, чай, вода, мука, масло, сыр
(+ milk/bread/sugar/water; запчасти: диск колесный/12345, фильтр масляный/77777).
