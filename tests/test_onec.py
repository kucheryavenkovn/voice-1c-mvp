...
def test_build_part_request_code_shape():
    ...
    # обязательные поля шапки Заказа на ремонт (проверка формы 1С)
    assert 'НайтиПоНаименованию("Текущий ремонт")' in code
    assert 'НайтиПоНаименованию("Средний")' in code
    assert "СтатусыЗаказовНаРемонт.Создан" in code
    assert "ХозяйственныеОперации.Ремонт" in code
    assert "ВариантыОбеспечения.СоСклада" in code
    assert "Рем.ДатаЗавершения = ТекущаяДата() + 604800" in code  # +неделя
    assert "СтрРем = Рем.Ремонты.Добавить();" in code
    assert "Стр.КодРемонта = 1;" in code
    # статья расходов строки «Ремонты»: Материалы для ремонта оборудования (коммерч.)
    assert 'НайтиПоКоду("ЦУ-000181")' in code
    assert "СтрРем.СтатьяРасходов = СтатьяРасх" in code
    # обязательные поля ЗаказаНаПеремещение
    assert "СтатусыВнутреннихЗаказов.КВыполнению" in code
    assert "СпособыДоставки.Самовывоз" in code
    assert "ЗП.ВариантПриемкиТоваров" in code
    assert "СтрЗ.ВариантОбеспечения = ВарОбесп" in code
