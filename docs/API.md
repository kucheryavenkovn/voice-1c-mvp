...
Интенты LLM (`X-Intent`): `get_stock` / `list_stock` (остатки),
`order_part` (заказ: `{"action":"order_part","item":"...","quantity":N,"warehouse":null}`),
`chat` (общие вопросы — краткий ответ из знаний модели, `{"action":"chat","answer":"..."}`),
`unknown` (фоллбэк-подсказка).
...
вода, мука, масло, сыр (+ англ. алиасы milk/bread/sugar/water; запчасти:
диск колесный/12345, фильтр масляный/77777). Незнакомый
товар → `found: false`.

### `POST /api/orders` — оформить заказ (кейс «заказ частей»)
body `{"item": "<товар>", "quantity": <целое>, "warehouse": "<склад>|null"}`.
Назначает номер `ЗР-NNNNNNN`, статус «Потребность зарегистрирована»;
товар с нулевым остатком → «заказ оформлен поставщику».
```json
{ "item": "уплотнитель", "found": true, "quantity": 3,
  "order_number": "ЗР-0001234", "status": "Потребность зарегистрирована",
  "message": "Создан заказ № ЗР-0001234: уплотнитель — 3 шт. Потребность зарегистрирована.",
  "source": "mock" }
```

### `GET /api/orders` — список созданных заказов
`{"orders": [{"number": "ЗР-0001234", "item": "...", "quantity": 3, ...}], "count": 1}`.
